...
def send_certificate_email(recipient_email, name, course):
    try:
        ...
        import uuid
        pdf_filename = f"{uuid.uuid4().hex}_certificate.pdf"
        tmp_pdf_path = os.path.join(tempfile.gettempdir(), pdf_filename)
        
        # Generate PDF with proper error handling
        try:
            pdfkit.from_string(html, tmp_pdf_path, configuration=config)
        except Exception as pdf_error:
            print(f"PDF generation error: {pdf_error}")
            # If PDF generation fails, still send an email without attachment
            msg = MIMEMultipart()
            msg['From'] = SENDER_EMAIL
            msg['To'] = recipient_email
            msg['Subject'] = 'Your Certificate of Completion'
            body = f"Dear {name},\n\nCongratulations on completing '{course}'! Due to technical issues, we couldn't generate your PDF certificate at this time. We'll send it in a separate email shortly.\n\nBest regards,\nMayur Dhoble"
        ...
        body = f"Dear {name},\n\nCongratulations! Please find your certificate of course completion of '{course}' attached as a PDF.\n\nBest regards,\nMayur Dhoble"
        msg.attach(MIMEText(body, 'plain'))

        # Add a slight delay to ensure the file is fully written and released
        time.sleep(0.5)
        
        # Try to attach the PDF with proper error handling
        try:
            ...
            print(f"Error attaching PDF: {file_error}")
            # If attaching fails, still send the email without attachment
            body = f"Dear {name},\n\nCongratulations on completing '{course}'! Due to technical issues, we couldn't attach your PDF certificate at this time. We'll send it in a separate email shortly.\n\nBest regards,\nMayur Dhoble"
            msg = MIMEMultipart()
            msg['From'] = SENDER_EMAIL
            msg['To'] = recipient_email
            msg['Subject'] = 'Your Certificate of Completion'
        ...
        # Clean up - try to delete with error handling
        try:
            # Add a delay before trying to delete
            time.sleep(1)
            if os.path.exists(tmp_pdf_path):
                os.unlink(tmp_pdf_path)
        except Exception as cleanup_error:
            print(f"Warning: Could not delete temporary PDF file {tmp_pdf_path}: {cleanup_error}")
...
def status():
    ...
    debug_data = {
        ...
        'sheet_id': os.environ.get("SHEET_ID", "Not set"),
        'smtp_server': SMTP_SERVER,
        'smtp_port': SMTP_PORT,
        'from_email': SENDER_EMAIL,
        'wkhtmltopdf_path': os.path.join(os.path.abspath(os.path.dirname(__file__)), 'wkhtmltopdf.exe'),
        'wkhtmltopdf_exists': os.path.exists(os.path.join(os.path.abspath(os.path.dirname(__file__)), 'wkhtmltopdf.exe')),
        'submissions': []
    }
    ...
    try:
        ...
        sheet = client.open_by_key(SHEET_ID).sheet1
        all_rows = sheet.get_all_values()
        debug_data['row_count'] = len(all_rows)
        ...
        debug_data['submissions'] = recent_submissions[-10:]
        
    except Exception as e:
        debug_data['sheets_error'] = str(e)
    
    # Check PDF generation
    try:
        with tempfile.NamedTemporaryFile(delete=False, suffix='.pdf') as test_pdf:
            config = pdfkit.configuration(wkhtmltopdf=debug_data['wkhtmltopdf_path'])
            pdfkit.from_string("<html><body>Test PDF</body></html>", test_pdf.name, configuration=config)
            os.unlink(test_pdf.name)  # Clean up the test file
    except Exception as e:
        debug_data['pdf_error'] = str(e)
    
    # Check email configuration
    try:
        with smtplib.SMTP(SMTP_SERVER, SMTP_PORT) as server:
            server.starttls()
            # Just check if we can connect, don't actually log in
    except Exception as e:
        debug_data['email_error'] = str(e)
    
    # Return debug template instead of regular status
    return render_template('error_status.html', **debug_data)
